...
int handle_cpu_event(void *ctx, void *data, uint64_t data_sz)
{
	...
	sprintf(log_string,
		"[CPU/MCE] CPU %d | Status: 0x%lx | Addr: 0x%lx | Misc: 0x%lx | Time: %lu ns\n",
...
int setup_cpu_monitor()
{
	...
	rb_cpu = ring_buffer__new(bpf_map__fd(cpu_skel->maps.mce_events), handle_cpu_event, NULL,
				  NULL);
	if (!rb_cpu) {
	...
	bpf_map__pin(cpu_skel->maps.mce_events,
		     "/sys/fs/bpf/radiation_testing_framework/map/mce_events");

	// pin all of the programs
	create_directory_if_not_exists("/sys/fs/bpf/radiation_testing_framework/prog");
	bpf_program__pin(cpu_skel->progs.trace_mce_event,
			 "/sys/fs/bpf/radiation_testing_framework/prog/trace_mce_event");
...
{
	// unpin the maps
	bpf_map__unpin(cpu_skel->maps.mce_events,
		       "/sys/fs/bpf/radiation_testing_framework/map/mce_events");
	// unping the programs
	bpf_program__unpin(cpu_skel->progs.trace_mce_event,
			   "/sys/fs/bpf/radiation_testing_framework/prog/trace_mce_event");
...
int add_cpu_config_key(const char *name, const char *value)
{
	...
	if (strcmp(name, "param1") == 0) {
		...
		if (!cpu_config->param2) {
			fprintf(stderr,
				"Failed to allocate memory for CPU handler config param2\n");
			return 0; // error
		}
